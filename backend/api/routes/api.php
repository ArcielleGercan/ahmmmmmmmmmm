...
// ==========================================
// DEBUG — remove after fixing puzzle stat
// ==========================================
Route::get('/debug/puzzle/{playerId}', function ($playerId) {
    try {
        $playerObjectId = new \MongoDB\BSON\ObjectId($playerId);

        // Check which database Laravel is actually using
        $model = new \App\Models\FastestTime();
        $model->setTable('fastest_time_puzzle');
        $connectionName = $model->getConnectionName();
        $dbName = $model->getConnection()->getDatabaseName();

        $all = \App\Models\FastestTime::queryPuzzle()->get();

        $byObjectId = \App\Models\FastestTime::queryPuzzle()
            ->where('player_id', $playerObjectId)->get();

        $byString = \App\Models\FastestTime::queryPuzzle()
            ->where('player_id', $playerId)->get();

        // Also try raw query to see what SQL/MQL is built
        $rawQuery = \App\Models\FastestTime::queryPuzzle()
            ->where('player_id', $playerObjectId)
            ->where('difficulty', 'EASY')
            ->where('category', 'Solar System');

        return response()->json([
            'laravel_connection'  => $connectionName,
            'laravel_database'    => $dbName,
            'player_id_input'     => $playerId,
            'total_in_collection' => $all->count(),
            'found_by_objectid'   => $byObjectId->count(),
            'found_by_string'     => $byString->count(),
            'found_all_conditions'=> $rawQuery->count(),
            'all_records'         => $all->map(function($r) {
                return [
                    '_id'        => (string) $r->_id,
                    'player_id'  => (string) $r->player_id,
                    'difficulty' => $r->difficulty,
                    'category'   => $r->category,
                    'time_secs'  => $r->time_seconds,
                ];
            }),
        ]);
    } catch (\Exception $e) {
...
Route::prefix('game')->group(function () {
    ...
    // "all-categories" literal MUST be before {category} wildcard
    ...
    // {category} wildcard AFTER the literal
    Route::get('/fastest-time/{playerId}/puzzle/{difficulty}/{category}', [FastestTimeController::class, 'getPlayerPuzzleFastestTimeByCategory']);

    // Generic game type route
...
Route::patch('/admin/players/{id}/restore', [AdminController::class, 'restorePlayer']);
